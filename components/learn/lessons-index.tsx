...
/**
 * The Learn index, rendered both at the global `/learn` route and the
 * repo-scoped `/[owner]/[repo]/learn` route. `basePath` is the prefix every
 * lesson link is built from so the same content works under either chrome.
 */
export async function LessonsIndex({ basePath }: { basePath: string }) {
          ...
          <section key={category.category} className="space-y-4">
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {category.lessons.map((lesson) => (
                <Link
                  key={lesson.slug}
                  href={`${basePath}/${category.category}/${lesson.slug}`}
                  className="group"
                >
